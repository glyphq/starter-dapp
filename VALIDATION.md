...
| Preserve WalletConnect request integrity      | `lib/connectors/wallet-connect.test.ts`                                                                               | The adapter rejects sessions without all requested methods or a valid mainnet identity, then adds the connected `{ from }` identity to signing and transaction inputs.                                |
| Reject malformed Glyph signing callbacks      | `lib/connectors/glyph-action-readiness.test.ts`                                                                       | A signed callback with a short or malformed signature is rejected before it becomes a connector result.                                                                                               |
| Keep pending task state and fields legible    | `scripts/qa-wallet-boundary.mjs` through the built UI                                                                 | A pending wallet request cannot dismiss its task dialog. Disabled fields visibly report a not-allowed state, and a failed message submission marks the affected field while the toast gives feedback. |
| Reset state-changing drafts on account change | `scripts/qa-wallet-boundary.mjs` through the built UI                                                                 | The real WalletProvider receives a synthetic `accountChanged` event. Lock QUs remounts with an empty amount, so a draft cannot carry into a different account.                                        |
...
| Static production output remains valid        | `bun run test && bun run build`                                                                                       | TypeScript and lint pass. The isolated suite passes 70 tests with 321 expectations. Next.js produces static `/` and `/_not-found` routes.                                                             |
